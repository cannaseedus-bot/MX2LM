...
def main(argv: List[str]) -> int:
    ap = argparse.ArgumentParser(
        prog="kuhul",
        description="KUHUL PoC — CLI ⇄ SVG (append-only events + replay)",
    )
    ...
    p_replay.add_argument(
        "--svg",
        default=None,
        help="Output svg filename. Default: <out>/<session>.replay.svg",
    )

    args = ap.parse_args(argv)

    ensure_dir(args.out)
    event_log_path = args.events or os.path.join(
        args.out, f"{args.session}.events.jsonl"
    )
    ...
    if args.cmd == "theme":
        cmd = make_cmd(
            idgen, session=args.session, op="ui.theme.apply", args={"name": args.name}
        )
    ...
    if args.cmd == "replay":
        replay_log_path = args.file or os.path.join(
            args.out, f"{args.session}.events.jsonl"
        )
        replay_svg = args.svg or os.path.join(
            args.out, f"{args.session}.replay.svg"
        )
